...
apply_rt_patches()
{
	...
	if [ $count -gt 0 ]; then
		...
		#make temporary copy of the Forecr's defconfig file
		cp ../arch/arm64/configs/dsboard_nx2_nano_defconfig\
			../arch/arm64/configs/.tmp.dsboard_nx2_nano_defconfig
		 ./config --file ../arch/arm64/configs/dsboard_nx2_nano_defconfig\
			--enable PREEMPT_RT_FULL \
			--disable DEBUG_PREEMPT \
			--disable CPU_IDLE_TEGRA18X \
			--disable CPU_FREQ_TIMES \
			--disable CPU_FREQ_GOV_SCHEDUTIL \
			--disable CPU_FREQ_GOV_INTERACTIVE || any_failure=1
		echo "PREEMPT RT patches successfully applied for DSBOARD_NX2!"
	fi
}
...
revert_rt_patches()
{
	...
	if [ $count -gt 0 ]; then
		...
		cp ../arch/arm64/configs/.tmp.dsboard_nx2_nano_defconfig\
			../arch/arm64/configs/dsboard_nx2_nano_defconfig
		rm -rf ../arch/arm64/configs/.tmp.dsboard_nx2_nano_defconfig
